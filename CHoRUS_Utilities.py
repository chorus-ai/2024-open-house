...
def read_waveform(folder_path, record):
    """
    Load the WF data from record (in this case a stand-alone .mat file).
    """
    # Define the path to the data (.mat) file.
    file_path = os.path.join(folder_path, record + '.mat')
    
    # Read waveform samples (input is in MAT format).
    mat_data = scipy.io.loadmat(file_path)

    # Select the dictionary entry with the waveform samples.
    samples = mat_data['val']

    # Flatten the data to a single dimensional array.
    data_arrray = np.squeeze(samples).astype(float)
...
def load_annotations(annot_path):
    """
    Read in the csv file with the annotation labels.
    """
    # Load the CSV file into a DataFrame.
    df = pd.read_csv(annot_path, header=None)
    
    # Map values in the second column to numerical representations for the classes.
    mapping = {'N': 0, 'A': 1, 'O': 2, '~': 3}
    df[2] = df[1].map(mapping)   
    
    # Rename columns as defined in the dict.
    new_column_names = {0: 'Record', 1: 'Label', 2: 'Class'}
...
def r_detection(ecg_signal, fs, wd=5, plot=False, preprocess=False):
    ...
    Parameters:
    ----------
    ecg_signal : Single-channel ECG.
    fs : Sampling frequency (1/s).
    wd : Window size (s) for the peak detection.
    plot : The option to plot, not recommended if the signal is long.
    preprocess : The option to begin by filtering the ECG signal with DWT.
    
    Returns:
    -------
    peaks_all_array : The numpy array of contains the location of R-peaks.
    recD : The filtered ECG signal.
    """
    # Create a list to hold the detected R-peaks.
    peaks_all_array = []

    # Filter the signal using the DWT from pywt package.
    if preprocess:
        # Define the level of decomposition for the DWT and the wavelet to use.
        level = 7
        Wtype = 'db4'
        
        # Perform the DWT, coverting the ECG into the coeffs.
        coeffs  = pywt.wavedec(ecg_signal, Wtype, level=level)

        # Unpack the approximation for coeffs. (cA7) and the other n coeffs. 
        cA7, cD7, cD6, cD5, cD4, cD3, cD2, cD1 = coeffs        

        # Set the coeffs. representng the parts of the WF we want to filter, to zero.
        Mapp = np.zeros_like(cA7)
        McD2 = np.zeros_like(cD2)
        McD1 = np.zeros_like(cD1)

        # Reconstruct the filtered ECG signal using the I-DWT.
    ...
    # Iterate over windows to find peak locations.
    for i in range(0, len(recD), l):
        window = recD[i:i+l]  # Extract windowed data
    ...
    # Post-processing peak correction.
    ...
    # Create the plot, if the options is set. 
    if plot:
...
def feature_extraction(x, fs):
    """
    Wrapper function to extract all 97 features from a waveform sample.
    """
    # Perform R-Peak detection and cleaning via CWT.
    R_i2, x = r_detection(x, fs, wd=5, plot=False, preprocess=True)

    # Calculate the statistical features from the P-wave, QRS-complex, QT interval, and ST-segment of the ECG signal.
    features_beat_0, beats = beat_features_v0(x, R_i2, fs)
    
    # Calculate the advanced beat features (Cosine Similarity, Autocorrelation, Eigenvalues, Zero Crossings, Gradient Analysis, Entropy).
    features_beat_1 = beat_features_v1(beats)
    
    # Calculate more advanced features from the signal (DWT, Zero Crossings, Autocorrelation, Gradient Analysis, Spectrogram Analysis...).
    adfeatures = advanced_features(x, fs)
    
    # Calculate yet more advanced features from the signal (Amplitude Variation, Proportion of Time Below Threshold, Frequency Domain Analysis).
    shfeatures = sh_vs_nshself(x, fs)
    
    # Compute a variety of Heart Rate Variability (HRV) features from the input RR intervals.
    hrvfeatures = hrv_features(R_i2, fs)

    # Concatenate the calculated features to be returned.
    all_features = np.hstack((features_beat_0, features_beat_1, adfeatures, shfeatures, hrvfeatures))
...
def beat_features_v0(xd1, R_i2, fs):
    """
    ...
    For each segment, it calculates statistical features, including PAR (positive area ratio), NAR (negative area ratio), md (mean of the first derivative), 
    vd (variance of the first derivative), and PP (peak-to-peak amplitude).
    """
    ...
    features_beat_0 = np.array([f1,  f2,  f3,  f4,  f5, 
                                f6,  f7,  f8,  f9,  f10, 
                                f11, f12, f13, f14, f15, 
                                f16, f17, f18, f19, f20])
    
    return features_beat_0, beats


def beat_features_v1(beats):
    """
    Input Validation: The function checks if the length of the input beats is greater than 3. If not, it returns an array of zeros.
    Interpolation: If there are more than 3 beats, the function interpolates each beat to 201 samples.
    This is done through a helper function interpolate_to_201_samples, which interpolates the beats to make them of equal length for further analysis.
    
    Cosine Similarity: The function calculates the cosine similarity for each pair of beats.
    Autocorrelation and Average Period: The function calculates autocorrelation and the average period for each pair of beats.
    Eigenvalues Analysis: It computes the covariance matrix of the interpolated beats and then calculates the eigenvalues of this covariance matrix.
    Zero Crossings: It identifies zero crossings in the sum of all interpolated beats (mbeats).
    Gradient Analysis: It computes the gradient of the sum of all interpolated beats (mbeats) and then the second gradient.
    Entropy Calculation: It computes the Shannon entropy of the distribution of amplitudes in the sum of all interpolated beats.
    """
...
def advanced_features(row1, fs):
    """
    Wavelet Transform decomposes the input signal using the Discrete Wavelet Transform (pywt.wavedec).
    Zero Crossings: It calculates the interquartile range (IQR) of the differences between consecutive zero crossings in the input signal.
    Autocorrelation: It calculates the autocorrelation of the input signal and finds the average period from the autocorrelation peaks.
    Gradient Analysis: It computes various statistics related to the gradient of the input signal.
    Spectrogram Analysis: It computes the spectrogram of the input signal and extracts various statistics from it.
    Instantaneous Frequency: It computes the instantaneous frequency using the Hilbert transform.
    Power Spectral Density (PSD): It computes the PSD of the input signal using the Welch method (signal.welch). 
        a. Power Band Features: It calculates maximum and mean power in different frequency bands.
    Seasonal Autocorrelation: It calculates seasonal autocorrelation at different time lags.
    Entropy: It computes the Shannon entropy of the input signal.
    Hjorth Parameters: It calculates Hjorth parameters: mobility (pfd) and complexity (kfd).
    Coefficient Distribution Homogeneity: It calculates the homogeneity of the distribution of continuous wavelet transform coefficients.
    Spectral Edge Frequency: It calculates the spectral edge frequency at different percentage levels.
    Burg Autoregression Coefficients: It computes the Burg AR coefficients.
    """
    ...
    features = np.array([TWE, iqr_1625, ha, hm, hc, 
                         np.var(cD5), np.var(cD4), np.var(cD3), 
                         average_period_1, se, fmax_stft, var_stft,
                         mins, sins, f1_psd, f2_psd, f3_psd, 
                         sa1, sa2, sa3,
...
def sh_vs_nshself(row, fs):
    """   
    Amplitude Variation (f1): It calculates the peak-to-peak amplitude of the filtered signal as a measure of amplitude variation.
    Proportion of Time Below Threshold (f2): It computes the proportion of time the squared first difference of the input signal is below a threshold (ThS). 
    Frequency Domain Analysis (f3): It performs a frequency domain analysis using the Fourier transform. 
    """
    lowcut = 6.5
    highcut = 30
    order = 5
...
def hrv_features(R_i2, fs):
    """
    NNx: The number of RR intervals differing by more than 50 ms.
    pNNx: The percentage of RR intervals differing by more than 50 ms.
    SD1: Standard deviation of beat-to-beat (NN) intervals in the Poincaré plot perpendicular to the identity line.
    SD2: Standard deviation of beat-to-beat (NN) intervals in the Poincaré plot along the line of identity.
    ratio_sd2_sd1: Ratio of SD2 to SD1.
    CSI (Cardiac Sympathetic Index): The ratio of the long-term variation (L) to short-term variation (T) in the NN intervals.
    CVI (Cardiac Vagal Index): A logarithmic transformation of the product of L and T.
    modifiedCVI: Modified CVI.
    f1: Root Mean Square of Successive Differences (RMSSD).
    f2: The ratio of the mean and standard deviation of NN intervals.
    sk_RR: Skewness of NN intervals.
    kurt_RR: Kurtosis of NN intervals.
    sampen_value: Sample entropy value.
    Features related to polynomial fit coefficients and geometric features: These include characteristics derived 
    from polynomial fitting and geometric properties of the RR interval scatter plot.
    """
    if len(R_i2) > 3:
...
def cross_validation(labels, n_folds=5, random_state=42, method="kfold"):
    ...
    if method == "kfold":
        ...
        # Method 2: stratified split (maintaining class member ratio) into 5 exclusive sets.
        train_folds, test_folds = [], []
